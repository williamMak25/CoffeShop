...
export const Products = () => {
  ...
  <div className='bg-cyan-700'>
    <div className='text-white bg-cyan-700'>
      ...
      <div className='flex flex-row justify-between items-center'> 
        <div className='flex flex-row justify-center items-center ml-3'>
          <li className={`list-none px-3 cursor-pointer ${filterBar === null ? 'bg-amber-700' : 'bg-cyan-700'} rounded-tl-lg border`} onClick={()=>setfilterBar("all")}>All</li>
          <li className={`list-none px-3 cursor-pointer ${filterBar === true ? 'bg-amber-700' : 'bg-cyan-700'} border`} onClick={()=>setfilterBar("bakery")}>Bakery</li>
          <li className={`list-none px-3 cursor-pointer ${filterBar === false ? 'bg-amber-700' : 'bg-cyan-700'} rounded-tr-lg border`} onClick={()=>setfilterBar("drink")}>Drink</li>
        </div>
    ...
    <div className='grid grid-cols-4 gap-2 justify-items-center p-2 bg-cyan-700 text-white p-3 max-md:grid-cols-3 max-sm:grid-cols-2'>
        ...
        {filerItems && filerItems?.map( item => {
          ...
          <div key={item?.id} className={filterBar || filterBar === null ?'w-full rounded-tr-3xl' : 'hidden'}>
            ...
            <>
              ...
              <div className='w-full flex flex-row justify-between bg-white text-black max-lg:flex-col'>
